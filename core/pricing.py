...
def obj_func(
    customers: np.ndarray, prices: float, demand_model: DemandModel, 
) -> float:
    """
    ...
    if prob_arr.shape[1] == 2:
        prob_arr = prob_arr[:, 1]
        return np.dot(prices, prob_arr) / prices.shape[0]
    else:  # prob_arr has shape (n_customers, n_bootstraps)
        return np.mean(prob_arr.T @ prices / prices.shape[0])
